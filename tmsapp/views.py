...
def user_login(request):
    if request.method == 'POST':
        ...
        form = LoginForm()
    return render(request, 'tmsapp/login.html', {'form': form})


@login_required
def user_tasks_list(request):
    tasks = request.user.tasks.all()
    return render(request, 'tmsapp/user_tasks_list.html', {'tasks': tasks})
...
def register(request):
    ...
    return render(request, 'tmsapp/register.html', {'form': form})
...
def create_task(request):
    if request.method == 'POST':
        ...
        return render(request, 'tmsapp/create_task.html', {'categories': categories, 'users': users})
...
def update_task(request, task_id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'tmsapp/update_task.html', {'task': task})
...
def complete_task(request, task_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('category_list')  # Redirect to a page where tasks are listed
    else:
        #messages.error(request, "Incomplete Task.")
        return redirect('category_list')  # Redirect to a page where tasks are listed
...
def category_list(request):
    categories = Category.objects.all()
    return render(request, 'tmsapp/category_list.html', {'categories': categories})
...
def create_category(request):
    if request.method == 'POST':
        ...
        return redirect('category_list')
    return render(request, 'tmsapp/create_category.html')
...
def category_tasks(request, category_id):
    ...
    return render(request, 'tmsapp/category_tasks.html', {'category': category, 'tasks': tasks})
...
def task_chart(request):
    ...
    return render(request, 'tmsapp/task_chart.html', {'pending_counts': pending_counts})
